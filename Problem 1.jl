...
# ╔═╡ d0c7fee0-1fa8-11f0-17a3-ed01780db7b0
using CSV, DataFrames, Statistics, Plots, Distances, Clustering, GaussianMixtures, MLJBase, StatsBase

# ╔═╡ 6dfeab51-fe3c-40c6-9a49-5109cd72f193
# Load the data
dia = CSV.read("C:/Users/s.ndongo/OneDrive - I-TECH Namibia/NUST/TAI911S/Assignment 2/dia.csv", DataFrame)

# ╔═╡ 4f661ac9-ea67-40cf-ba62-a851b79e1037
# Inspect the data
println("Drug-Induced Autoimmune Data Shape: ", size(dia))

# ╔═╡ c96a881b-a28f-4c74-883a-b1138b5804df
# Data Cleaning by handling missing values
begin
    dropmissing!(dia)
    numeric_cols = names(dia, eltype.(eachcol(dia)) .== Float64)
    dia_cleaned = dia[:, numeric_cols]
    first(dia_cleaned, 5)
end

# ╔═╡ 5ea7b56e-08bf-4d54-b4bc-906a57b39255
# Scaling
begin
    function safe_zscore(col)
        valid = skipmissing(col)
        if std(valid) == 0
            return fill(0.0, length(col))
        else
            return zscore(col)
        end
    end

    for col in names(dia_cleaned)
        dia_cleaned[!, col] = safe_zscore(dia_cleaned[!, col])
    end
    Matrix(dia_cleaned)
end

# ╔═╡ 983bd613-e07b-42d9-9e4d-47d9a05ff487
# K-Means:
begin
    k = 3  # or any chosen cluster count
    dia_matrix = Matrix(dia_cleaned)
    kmeans_result = kmeans(dia_matrix, k)

    # Compute pairwise distance matrix once
    dist_matrix = pairwise(Euclidean(), dia_matrix, dims=2)

    # Now compute silhouette scores
    sil_scores = silhouettes(kmeans_result.assignments, dist_matrix)
    silhouette_kmeans = mean(sil_scores)
    silhouette_kmeans
end

# ╔═╡ c71a94cf-17e0-4de0-acec-9c97fc748783
# Agglomerative Clustering:
begin
    hclust_result = hclust(pairwise(Euclidean(), dia_matrix, dims=2), linkage=:ward)
    agg_assignments = cutree(hclust_result, k=3)

    sil_scores_agg = silhouettes(agg_assignments, dist_matrix)
    silhouette_agg = mean(sil_scores_agg)
    silhouette_agg
end

# ╔═╡ 56d0a094-a7fb-43de-afcc-3c7551c3723e
# Gaussian Mixture:
...
manifest_format = "2.0"
project_hash = "2e066f7ce362c908ef58ab6ce8a98f9afd09fade"
